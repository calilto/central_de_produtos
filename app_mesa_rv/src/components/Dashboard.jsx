...
import {
  ...
  ChevronDown, RefreshCw, Landmark, Percent, AlertTriangle, Info
} from 'lucide-react';
...
export default function Dashboard() {
  ...
  // --- HELPER: CALCULATE PUT DO PARAMS ---
  const getPutDoParams = (p) => {
    // 1. Premium is technically the abs value of breakEvenDrop (assuming 1:1 leverage start)
    //    OR we solve for both if leverage isn't 1.
    // Let's use the Calibration Points to solve for Leverage.
    // Formula: Return = ( (Abs(Asset) * Lev) - Premium ) / Premium
    // We know:
    // Point 1 (BreakEven): Return = 0 at Asset = p.breakEvenDrop
    //    0 = (Abs(breakEven) * Lev) - Premium  => Premium = Abs(breakEven) * Lev
    // Point 2 (Max): Return = maxReturnPct/100 at Asset = p.maxReturnDrop
    //    maxReturnPct/100 = ( (Abs(maxDrop) * Lev) - Premium ) / Premium
    // Substitute Premium:
    //    maxReturnPct/100 = ( (Abs(maxDrop) * Lev) - (Abs(breakEven) * Lev) ) / (Abs(breakEven) * Lev)
    //    Lev cancels out!
    //    maxReturnPct/100 = ( Abs(maxDrop) - Abs(breakEven) ) / Abs(breakEven)
    // This means if the user enters consistent data, Leverage is 1.0 multiplier on the underlying logic.
    // However, if the product has structural leverage (e.g. 2x participation), the "Asset" in the formula is effective asset.

    // Let's assume standard Put DO structure where Cost = BreakEvenDrop (approx).
    // Then we calculate implied Leverage from the Max Point to be safe.
    // ImpliedLev = (Premium * (1 + Return)) / Abs(AssetDrop)

    const premiumEst = Math.abs(p.breakEvenDrop); // Initial guess
    const targetReturn = p.maxReturnPct / 100;
    const absMaxDrop = Math.abs(p.maxReturnDrop);

    // Lev = (Premium * (1 + ROI)) / Drop
    const impliedLeverage = (premiumEst * (1 + targetReturn)) / absMaxDrop;

    return { premium: premiumEst, leverage: impliedLeverage };
  };

  // --- PAYOFF CALCULATION (CORE) ---
  const calculateReturn = (variation) => {
    ...
    switch (selectedStrategy) {
      case 'collar_ui_bi':
        const pisoBi = -(100 - p.protection); // se protection = 110, pisoBi = +10
        if (v === 0) {
          ...
          if (v <= p.downsideBarrier) {
            ret = Math.max(0, pisoBi);
            scenario = "Proteção Total (KO Baixa) + " + (ret > 0 ? `Piso Mínimo (${p.protection}%)` : "Prêmio Dzerado");
          }
          else {
      ...
      case 'collar_ui':
        const maxLoss = -(100 - p.protection); // se protection for 110, maxLoss = +10
        if (v < 0) {
      ...
      case 'put_do':
        // Put DO with Calibration
        const { premium, leverage } = getPutDoParams(p);
        ...
        if (v > 0) {
          ...
          if (v <= p.knockOutBarrier) {
            ...
            // Formula: ((Abs(V) * Lev) - Premium) / Premium
      ...
      case 'cupom':
        const accumulatedCoupons = p.monthlyCoupon * simulatedCouponsReceived;
        if (v >= p.barrier) {
  ...
  const chartData = useMemo(() => {
    ...
    if (selectedStrategy === 'doc_bi') {
      addP(params.protectionBarrier);
      addP(params.upsideCap);
    } else if (selectedStrategy === 'put_do') {
      ...
      if (params.protection) {
        if (selectedStrategy === 'fence_ui') addP(-(100 - params.protection));
        if (selectedStrategy === 'pop') addP(-(100 - params.protection));
        if (selectedStrategy === 'collar_ui') addP(-(100 - params.protection));
        if (selectedStrategy === 'collar_ui_bi') addP(-(100 - params.protection));
      }
    }

    return dataPoints.sort((a, b) => a - b).map(val => {
      const res = calculateReturn(val);
      return {
        variation: val,
  ...
  const simulationResults = useMemo(() => {
    ...
    if (selectedStrategy === 'put_do') {
      const res = calculateReturn(simulatedVariation);
      retPct = res.returnPct; // ROI%
      scenario = res.scenario;
      const premiumCost = investment; // Investment is the Premium
      profit = premiumCost * (retPct / 100);
      valFinal = premiumCost + profit;

    } else if (selectedStrategy === 'cupom') {
      const assetResult = simulatedVariation < params.barrier ? simulatedVariation : 0;
      const couponGain = simulatedCouponsReceived * params.monthlyCoupon;
      retPct = couponGain + assetResult;
      scenario = simulatedVariation < params.barrier
        ? `Ativo < Barreira (${params.barrier}%) + ${simulatedCouponsReceived} Cupons`
        : `Capital Devolvido + ${simulatedCouponsReceived} Cupons`;
          ...
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="flex items-center gap-2 px-3 py-2 rounded-md text-sm font-bold text-gray-400 hover:text-white transition-all border border-transparent hover:border-gray-800"
              >
                Sair
              </button>
            </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <h2 className="text-3xl font-bold text-white flex items-center gap-3">
                  ...
                  <span className="text-yellow-500">{STRATEGIES[selectedStrategy].name}</span>
                </h2>
                <p className="text-gray-400 text-sm mt-1">{STRATEGIES[selectedStrategy].description}</p>
              </div>
                ...
                <Card className="p-5 h-full flex flex-col justify-between bg-gradient-to-b from-gray-900 to-gray-950">
                  <div>
                    ...
                    <div className="mb-8">
                      ...
                      <input
                        ...
                        onChange={(e) => setSimulatedVariation(Number(e.target.value))}
                        className="w-full h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer accent-yellow-500"
                      />
                      <div className="flex justify-between text-[10px] text-gray-600 mt-2 font-mono">
                        <span>-60%</span>
                        <span className="cursor-pointer hover:text-white" onClick={() => setSimulatedVariation(0)}>0%</span>
                        <span>+70%</span>
                      </div>
                    </div>

                    {/* Special Control: Cupom Recorrente */}
                    {selectedStrategy === 'cupom' && (
                      <div className="mb-8 p-3 bg-gray-800/50 rounded border border-gray-700 ring-2 ring-blue-500/20">
                        ...
                        <input
                          ...
                          onChange={(e) => setSimulatedCouponsReceived(Number(e.target.value))}
                  ...
                  <div className="bg-gray-800/30 rounded-lg p-4 border border-gray-700/50">
                    ...
                    <div className="grid grid-cols-2 gap-4 pt-4 border-t border-gray-700/50">
                      <div className={`p-2 rounded ${simulationResults.retPct > 0 && selectedStrategy.includes('collar') && params.protection > 100 && simulationResults.retPct === -(100 - params.protection) ? 'bg-green-900/30 border border-green-500/50 shadow-[0_0_10px_rgba(34,197,94,0.2)]' : ''}`}>
                        ...
                        {selectedStrategy.includes('collar') && params.protection > 100 && simulationResults.retPct === -(100 - params.protection) && (
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-700/50 flex justify-between items-center">
                      <span className="text-[10px] text-gray-500 uppercase">
                        {selectedStrategy === 'put_do' ? 'Lucro/Prejuízo Líquido' : 'Resultado R$'}
                      </span>
                  ...
                  <div className="flex-1 w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <ComposedChart data={chartData} margin={{ top: 20, right: 20, left: 0, bottom: 20 }}>
                        ...
                        <Area type="monotone" dataKey="strategyReturn" stroke="#EAB308" fill="url(#colorStrategy)" strokeWidth={3} isAnimationActive={false} />
                      </ComposedChart>
                    </ResponsiveContainer>
                  </div>
                </Card>
